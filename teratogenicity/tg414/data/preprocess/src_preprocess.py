...
#%%
with open('tg414_page_src.json', 'r') as file:
...
def admin_type(string):
    if check_nan(string):
        ...
        if 'oral' in string and 'dermal' in string and 'inhalation' in string:
            type_ = 'all'
        
        elif 'inhalation' in string:
            type_ = 'inhalation'
        
        elif 'oral' in string or 'liquid diet' in string:
            type_ = 'oral'
        
        elif 'dermal' in string or 'intravenous' in string:
            type_ = 'dermal'
...
df['admin type'] = df['Route of administration'].map(lambda x: admin_type(x))

# if inhale type is nan then replace the value as maximal occurance of type
df['admin type'][df['admin type'].isna()] = df['admin type'].value_counts(sort=True).index[0]

# save df
df.to_excel('tg414_raw.xlsx', header = True, index = False)
