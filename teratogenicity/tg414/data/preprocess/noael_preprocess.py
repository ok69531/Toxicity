...
#%%
df_tmp = pd.read_excel('tg414_raw.xlsx', header = 0)
...
noael['Effect level'][other_idx] = [re.sub('other: ', '', noael['Effect level'][i]).strip() for i in other_idx]


# Value에 ca. ~ 이렇게 돼있는거에서 ca. 제거
ca_idx = [i for i in range(len(noael)) if 'ca.' in str(noael['Effect level'][i])]
noael['Effect level'][ca_idx] = [re.sub('ca\. ', '', noael['Effect level'][i]).strip() for i in ca_idx]
...
def unit_transform(string):
    ...
    elif 'ml/kg' in string:
        unit_ = re.sub('ml/kg|mL/kg', 'ml/kg', string)
...
noael['Effect level'] = noael['Effect level'].map(lambda x: re.sub('bw/day', '', unit_transform(str(x))).strip())
...
# 단위
# val_df['unit'] = ''
u_ = ['µg/m^3', 'mg/m^3', 'g/m^3', 'mg/L', 'ppm', 'ml/kg', 'mg/kg', 'ml/kgbw/day', 'mg/kgbw/day']
...
val_df.to_excel('tg414_noael.xlsx', header = True, index = False)
