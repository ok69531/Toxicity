...
#%%
df_tmp = pd.read_excel('tg413_raw.xlsx', header = 0)

df_tmp['Dose descriptor'].unique()
df_tmp['Dose descriptor'].value_counts()

noael = df_tmp[df_tmp['Dose descriptor'].isin(['NOAEL', 'NOAEC', 'NOEC', 'NOEL', 'other: NOEL (rats)'])].reset_index(drop = True)
...
val_df.to_excel('tg413_noael.xlsx', header = True, index = False)
