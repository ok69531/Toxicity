...
#%%
lc50_tmp = pd.read_excel('echa_inhalation.xlsx', header = 0)
...
# Value datframe
val_df = pd.DataFrame(columns = {'CasRN', 'Value_tmp', 'Value_split', 'lower_ineq', 'lower_value', 
                                 'upper_ineq', 'upper_value', 'unit', 'air', 'nominal/analytical'}) 
val_df.columns = ['CasRN'] + ['Value_tmp'] + ['Value_split'] + ['lower_ineq'] + ['lower_value'] + ['upper_ineq'] + ['upper_value'] + ['unit'] + ['air'] + ['nominal/analytical']
...
for i in idx:
    if isFloat(''.join(val_df['Value_split'][i][:3])):
        val_df['lower_value'][i] = ''.join(val_df['Value_split'][i][:3])
        del val_df['Value_split'][i][:3]
    
    elif isFloat(''.join(val_df['Value_split'][i][:2])):
        val_df['lower_value'][i] = ''.join(val_df['Value_split'][i][:2])
        del val_df['Value_split'][i][:2]
    
    elif isFloat(''.join(val_df['Value_split'][i][:1])):
        val_df['lower_value'][i] = ''.join(val_df['Value_split'][i][:1])
...
for i in range_idx:
    if val_df['Value_split'][i].index('-') == 1:
        val_df['lower_value'][i] = val_df['Value_split'][i][0]
        # val_df['Value_split'][i].remove(val_df['Value_split'][i][0])
        
        if isFloat(''.join(val_df['Value_split'][i][2:4])):
            val_df['upper_value'][i] = ''.join(val_df['Value_split'][i][2:4])
            # val_df['Value_split'][i].remove(''.join(val_df['Value_split'][i][2:4]))
        else:
            val_df['upper_value'][i] = val_df['Value_split'][i][2]
            # val_df['Value_split'][i].remove(val_df['Value_split'][i][2])
        
            
    elif val_df['Value_split'][i].index('-') == 2:
        val_df['lower_value'][i] = ''.join(val_df['Value_split'][i][:2])
        val_df['upper_value'][i] = ''.join(val_df['Value_split'][i][3:5])
        
        # del val_df['Value_split'][i][:2]
        # del val_df['Value_split'][i][3:5]
...
val_df.to_excel('echa_inhalation_preprocessed.xlsx', header = True, index = False)
